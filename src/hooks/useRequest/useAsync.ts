...
export function useAsync<R, P extends any[], U, UU extends U>(
  ...
  const run = (...args: P) => {
    ...
    if (!curFetch) {
      const fetch = new Fetch(serve, fetchConfig, subscribe.bind(null, curFetchKey.value), {
        loading: defaultLoading,
        params: initialParams,
        data: initialData,
      });
...
): [Partial<CurFetchResult<R, P>>, (args: Partial<CurFetchResult<R, P>>) => void] {
  const fetchResult = shallowReactive<Partial<CurFetchResult<R, P>>>(initalValue);
  const setFetcResult = ({
    data,
    loading,
    params,
