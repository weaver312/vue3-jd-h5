 ...
 * @LastEditTime: 2021-03-20 17:59:41
 * @FilePath: /vue-jd-h5/src/hooks/useDrop/useDrop.ts
 */
...
type CallbackFn = (dataTransfer: DataTransfer, evt: DragEvent | ClipboardEvent) => any;
...
export function useDrop(options: DropOpt = {}): [DropProps, ComputedRef<boolean>] {
  ...
  const callback: CallbackFn = (dataTransfer, evt) => {
    ...
    if (dataTransfer.files && dataTransfer.files.length > 0 && options?.onFiles) {
      options.onFiles([...Array.from(dataTransfer.files)], evt as DragEvent);
      return;
    }
    if (dataTransfer.items && dataTransfer.items.length > 0 && options?.onText) {
